        ...
        createConnection: (x: any) => {
            return {
                query: (a: any, b: any, cb: any) => {
                     ...
                     if(a.includes('SELECT PT.')) {
                        return cb(null, []);
                    } else if (a.includes('SELECT ')) {
                        return cb(null, [{
                            id_estado: 123,
                            nombre_estado: 'LISTO PARA FACTURAR',
                            estado_transporte: 1,
                            id_transporte: 1,
...
describe('Lambda Listo Para Facturar', () => {

    test('Camino Feliz Exito', async () => {
        let rawdata = fs.readFileSync('listo_para_facturar.json');
